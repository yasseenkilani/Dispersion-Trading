"""
Dispersion Trading - Daily Runner
==================================

Main script to run daily for dispersion trading.

HYBRID STRATEGY:
- SHORT signals: ML-enhanced (66.9% win rate)
- LONG signals: Z-score only (ML doesn't add value for LONG)

Tracks both Z-score and ML strategies in parallel for comparison.

Usage:
    python run_daily.py              # Full run with IBKR
    python run_daily.py --offline    # Test mode without IBKR
    python run_daily.py --signal-only # Generate signal only, no trading
"""
...
from paper_trader import run_trading_session, DualPortfolioManager
...
def save_intraday_data(signal, ml_prediction, iv_data):
    ...
    This data is collected but NOT used for ML training to avoid overfitting.
    Use cases:
    - Analyze intraday IV/correlation patterns
    - Identify optimal execution timing
    - Validate signal consistency throughout the day
    """
    import json
...
def main():
    """Main entry point for daily dispersion trading."""
    parser = argparse.ArgumentParser(description='Dispersion Trading Daily Runner (QQQ-based)')
    ...
    print("\n" + "=" * 70)
    print("DISPERSION TRADING SYSTEM - HYBRID ML")
    print("=" * 70)
    ...
    if args.log_data:
        ...
        # Execute trades for both strategies if not signal-only mode
        if not args.signal_only:
            run_trading_session(signal, ml_prediction)
        else:
